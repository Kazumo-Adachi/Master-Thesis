...
#define FLUME_ID    3       /* 水路形状: 1=直線, 2=起伏小, 3=起伏大 */
#define SLOPE_VALUE    20       /* 勾配: 20度, 30度 */
/* ================================================================ */
...
#define OUTPUT_DIR       "FIX堀田/" FLUME_NAME "/"
...
int main(void) {
    ...
    if (fho == NULL || fzo == NULL || fMo == NULL || fuo == NULL ||
        ...
        fslopeo == NULL || fPexo == NULL || fPmaxo == NULL || fPmino == NULL || 
        fM200o == NULL) {
        printf("出力ファイルエラー\n");
        printf("  出力先: %s\n", OUTPUT_DIR);
    ...
    fclose(ftaui); fclose(ftauo); fclose(fslopeo); fclose(fflume); 
    fclose(fPexo); fclose(fPmaxo); fclose(fPmino);
    fclose(fM200o);
