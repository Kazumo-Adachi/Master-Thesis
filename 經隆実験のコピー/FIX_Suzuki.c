...
#define OUTPUT_DIR       "FIX鈴木/" FLUME_NAME "/"
...
const double G_e = 0.775;
const double G_tanfais = 0.795436;
const double G_cstar = 0.70;
...
double eval_he_equation(double Ce, double he, double M, double slope_E, double d, double cs) {
    ...
    double ro_m_e = G_cstar * (G_sigma - G_ro) + G_ro; //タイプミスかも
    ...
    for (n = 0; n < T_GRID; n++) {
        ...
        for (i = 1; i < X_GRID; i++) {
            ...
            if (h2[i] < stop) { tau[i] = 0; M2[i] = 0; u2[i] = 0; E[i] = 0; z2[i]=z1[i]; }
                ...
                if(slope < 0){
                    ...
                    Ce = ro * tan(slope) / (sigma - ro) / (tanfais - tan(slope));
                    if (Ce < 0) Ce = 0;
                    if (Ce >= cstar) Ce = cstar - EPS;
                }
                double slope_E = slope;
                if (slope_E < 0) {
                    slope_E = atan(((z1[i]+h2[i])-(z1[i+1]+h2[i+1]))/dx);
                }

                double he;
                if(Ce==0){he=0;}
                else{
                    he = calc_he_bisection(Ce, fabs(M2[i]), slope_E, d, cs, 0.01, 1000.0);
                }
        ...
        for (i = 1; i < X_GRID; i++) {
            ...
            if (h1[i] < stop) { tau[i] = 0; M1[i] = 0; u1[i] = 0; E[i] = 0;z1[i]=z2[i]; }
                ...
                double slope_E = slope;
                if (slope_E < 0) {
                    slope_E = atan(((z2[i]+h1[i])-(z2[i+1]+h1[i+1]))/dx);
                }
        
                double he;
                if(Ce==0){he=0;}
                else{
                    he = calc_he_bisection(Ce, fabs(M1[i]), slope_E, d, cs, 0.01, 1000.0);
                }
                if (he <= 0 || isnan(he) || isinf(he)) {
