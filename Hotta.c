...
int main(void) {
    ...
    fho = fopen("堀田/H_case" CASE_ID_STR "/堀田ho" CASE_ID_STR ".txt", "w");
    ...
    fzo = fopen("堀田/H_case" CASE_ID_STR "/堀田zo" CASE_ID_STR ".txt", "w");
    ...
    fEo = fopen("堀田/H_case" CASE_ID_STR "/堀田Eo" CASE_ID_STR ".txt", "w");
    ...
    ftauo = fopen("堀田/H_case" CASE_ID_STR "/堀田tau" CASE_ID_STR ".txt", "w");
  ...
  fMo = fopen("堀田/H_case" CASE_ID_STR "/堀田Mo" CASE_ID_STR ".txt", "w");
  if (fMi == NULL || fMo == NULL) {
    ...
    fuo = fopen("堀田/H_case" CASE_ID_STR "/堀田uo" CASE_ID_STR ".txt", "w");
    ...
    fco = fopen("堀田/H_case" CASE_ID_STR "/堀田co" CASE_ID_STR ".txt", "w");
    ...
    /* slope(θ)の結果用ファイル */
    fslopeo = fopen("堀田/H_case" CASE_ID_STR "/堀田slopeo" CASE_ID_STR ".txt", "w");
    if (fslopeo == NULL) {
        printf("error slope\n");
        exit(1);
    }
    ...
    /* 過剰圧 */
    fPexo = fopen("堀田/H_case" CASE_ID_STR "/堀田Pexo" CASE_ID_STR ".txt", "w");
    if (fPexo == NULL) {
        printf("error Pex\n");
        exit(1);
    }
    /* 過剰圧最大値 */
    fPmaxo = fopen("堀田/H_case" CASE_ID_STR "/堀田Pmaxo" CASE_ID_STR ".txt", "w");
    if (fPmaxo == NULL) {
        printf("error Pmax\n");
        exit(1);
    }
    /* 過剰圧最小値 */
    fPmino = fopen("堀田/H_case" CASE_ID_STR "/堀田Pmino" CASE_ID_STR ".txt", "w");
    if (fPmino == NULL) {
        printf("error Pmin\n");
        exit(1);
    }
    /* 粒子レイノルズ数 */
    fRepo = fopen("堀田/H_case" CASE_ID_STR "/堀田Repo" CASE_ID_STR ".txt", "w");
    if (fRepo == NULL) {
        printf("error Rep\n");
        exit(1);
    }
    ...
    /* 堆積厚さD = z - flume */
    fDo = fopen("堀田/H_case" CASE_ID_STR "/堀田Do" CASE_ID_STR ".txt", "w");
    if (fDo == NULL) {
        printf("error D\n");
        exit(1);
    }
    ...
    /* E0（過剰圧考慮前の侵食速度） */
    fE0o = fopen("堀田/H_case" CASE_ID_STR "/堀田E0o" CASE_ID_STR ".txt", "w");
    if (fE0o == NULL) {
        printf("error E0\n");
        exit(1);
    }
    ...
    /* 時系列データ用CSVファイル（i=126, 134, 138） */
    fts_126 = fopen("堀田/H_case" CASE_ID_STR "/timeseries_i126_" CASE_ID_STR ".csv", "w");
    if (fts_126 == NULL) {printf("error timeseries i=126\n");exit(1);}
    fprintf(fts_126, "n,time,c,h,Rep,Pex,Pmax,Pmin,D\n");

    fts_134 = fopen("堀田/H_case" CASE_ID_STR "/timeseries_i134_" CASE_ID_STR ".csv", "w");
    if (fts_134 == NULL) {printf("error timeseries i=134\n");exit(1);}
    fprintf(fts_134, "n,time,c,h,Rep,Pex,Pmax,Pmin,D\n");

    fts_134_txt = fopen("堀田/H_case" CASE_ID_STR "/timeseries_i134_" CASE_ID_STR ".txt", "w");
    if (fts_134_txt == NULL) {printf("error timeseries txt i=134\n");exit(1);}

    fts_138 = fopen("堀田/H_case" CASE_ID_STR "/timeseries_i138_" CASE_ID_STR ".csv", "w");
    if (fts_138 == NULL) {printf("error timeseries i=138\n");exit(1);}
    fprintf(fts_138, "n,time,c,h,Rep,Pex,Pmax,Pmin,D\n");
    ...
    for (n = 0; n < T_GRID; n++) {
        ...
        for (i = 1; i < X_GRID; i++) {
            ...
            if (h2[i] < stop ) {tau[i] = 0; M2[i] = 0; u2[i] = 0; E[i] = 0; slope_arr[i] = slope;}
                ...
                double E0 =fabs(u2[i])*tan(slope-theta_0);

                if(c2[i] <= c_min && E0<0) E0=0;

                if (E0>0){
                    double dist = z2[i] - flume[i];
                    if(dist<0){E0=0;}
                    else if(dist < E0*dt){ E0=dist/dt;}
                    else{E0=E0;}
                }
                E0_arr[i] = E0;  /* E0を配列に保存 */

                if (c2[i] > c_min){
                    //平衡勾配式
                    double A = (c2[i] * (sigma - ro) + ro)* g * h2[i];
                    double B = c2[i] * (sigma - ro) * g * h2[i];
                    double Rep = (1-c2[i])*ro*d*fabs(E0)/mu;
                    Rep_arr[i]=Rep;

                    double Pmax = B*cos(slope);
                    double Pmin = -ro*g*h2[i]*cos(slope);
                    Pmax_arr[i]=Pmax/ro/g/cos(slope);
                    Pmin_arr[i]=Pmin/ro/g/cos(slope);
                    ...
                    Dar = 18*mu*C_sub /d/ d;
                    For = 27*ro*C_sub /8 /d;
                    Pex = Dar*h2[i]*(-E0)+For*h2[i]*(-E0)*fabs(E0);                    
                    Pex_arr[i]=Pex/ro/g/cos(slope);

                    if (Pex/Pmax>=1) Pex =Pmax; //数値実装上のクリップ
                    if (Pex-Pmin <= EPS) Pex =Pmin; //数値実装上のクリップ

                    //三角関数の合成
                    double R=sqrt(A*A+B*B*tanfais*tanfais);
                    double delta=-Pex*tanfais/R;
                    if(delta<=-1)delta=-1; 
                    if(1<=delta)delta=1;

                    double theta_now = asin(delta) + theta_0;
                    if((slope-theta_0)*(slope-theta_now)<0){E[i] =0;}
                    else{
                        E[i] = fabs(u2[i]) * tan(slope - theta_now);
                    }
                }
                else{
                    E[i] = E0;
            ...
            //fprintf(fEo, "%f ", E[i]);
            /*過剰圧*/
            //fprintf(fPexo, "%f ", Pex_arr[i]);
            //fprintf(fPmaxo, "%f ", Pmax_arr[i]);
            //fprintf(fPmino, "%f ", Pmin_arr[i]);
            //fprintf(fRepo, "%f ", Rep_arr[i]);
            //fprintf(fslopeo, "%f ", slope_arr[i]);
            /* 堆積厚さD, E0 */
            //fprintf(fDo, "%f ", D_arr[i]);
            //fprintf(fE0o, "%f ", E0_arr[i]);
        }
        ...
        for (i = 1; i < X_GRID; i++) {
            ...
            if (h1[i] < stop ) {tau[i] = 0; M1[i] = 0; u1[i] = 0; E[i] = 0; slope_arr[i] = slope;}
                ...
                double E0=fabs(u1[i])*tan(slope-theta_0);

                if(c1[i] <= c_min && E0<0) E0=0;
                ...
                if (c1[i] > c_min){
                    //平衡勾配式
                    double A = (c1[i] * (sigma - ro) + ro)* g * h1[i];
                    double B = c1[i] * (sigma - ro) * g * h1[i];
                    double Rep = (1-c1[i])*ro*d*fabs(E0)/mu;
                    Rep_arr[i]=Rep;

                    double Pmax = B*cos(slope);
                    double Pmin = -ro*g*h1[i]*cos(slope);
                    Pmax_arr[i]=Pmax/ro/g/cos(slope);
                    Pmin_arr[i]=Pmin/ro/g/cos(slope);
                    ...
                    Dar = 18*mu*C_sub /d/ d;
                    For = 27*ro*C_sub /8 /d;
                    Pex = Dar*h1[i]*(-E0)+For*h1[i]*(-E0)*fabs(E0);
                    Pex_arr[i]=Pex/ro/g/cos(slope);
                    ...
                    if((slope-theta_0)*(slope-theta_now)<0){E[i] =0;}
                        ...
                        E[i] = fabs(u1[i]) * tan(slope - theta_now);
                    }
                }
                else{
                    E[i] = E0;
            ...
            /*過剰圧*/
            if(n % Record == Record - 1 ){fprintf(fPexo, "%f ", Pex_arr[i]);}
            ...
            if(n % Record == Record - 1 ){fprintf(fE0o, "%f ", E0_arr[i]);}
        ...
        if(n % Record == Record - 1 ){fprintf(fE0o, "\n");}
    ...
    printf("堀田RMSE (final z vs experiment) = %f\n", RMSE);

    /* --- 最終時刻で z[i] - flume[i] > d となる範囲の幅を計算（計算結果） --- */
    double D = 0.0; 
    double i_D = 0.0;
    for (i = 1; i < X_GRID; i++) {
        if (z1[i] - flume[i] >= d) {
    ...
    if (i_start == -1) {
        ...
        printf("堀田計算 堆積開始地点:X_start=%f, 堆積到達地点:X_end=%f\n", X_start, X_end);}
        /* ついでに幅も欲しければ */
    ...
    printf("----- 計算完了しました（堀田case" CASE_ID_STR "） -----\n");
    printf("----- 時系列データCSV: 堀田/H_case" CASE_ID_STR "/timeseries_i126_" CASE_ID_STR ".csv -----\n");
    printf("----- 時系列データCSV: 堀田/H_case" CASE_ID_STR "/timeseries_i134_" CASE_ID_STR ".csv -----\n");
    printf("----- 時系列データCSV: 堀田/H_case" CASE_ID_STR "/timeseries_i138_" CASE_ID_STR ".csv -----\n");
    printf("----- 堆積厚さD: 堀田/H_case" CASE_ID_STR "/堀田Do" CASE_ID_STR ".txt -----\n");
    printf("----- E0: 堀田/H_case" CASE_ID_STR "/堀田E0o" CASE_ID_STR ".txt -----\n");
    return 0;
}