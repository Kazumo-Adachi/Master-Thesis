...
#define Q_ID        4       /* 流量: 1=Q_STD, 2=Q_HIGH */
/* ================================================================ */
...
#define OUTPUT_DIR       "鈴木堆積/SD_" GRAIN_NAME "_" FLUME_NAME "_" Q_NAME "/"
#define OUTPUT_PREFIX    "鈴木堆積_" GRAIN_NAME "_" FLUME_NAME "_" Q_NAME "_"
...
int main(void) {
    ...
    c1[0] = co; c1[X_GRID] = 0;
    ct[0] = co; ct[X_GRID] = 0;
    z1[0] = TOP; z1[X_GRID] = -0.17;

    h2[0] = ho; h2[X_GRID] = 0;
    M2[0] = Mo; M2[X_GRID] = 0;
    u2[0] = uo; u2[X_GRID] = 0;
    c2[0] = co; c2[X_GRID] = 0;
    z2[0] = TOP; z2[X_GRID] = -0.17;

    E[0] = 0; E[X_GRID] = 0;
    tau[0] = 0; tau[X_GRID] = 0;
    flume[0] = TOP; flume[X_GRID] = -0.17;
    ...
    for (n = 0; n < T_GRID; n++) {
        ...
        for (i = 1; i < X_GRID; i++) {
            ...
            if (h2[i] < stop) { 
                ...
                if (slope_E < 0) {
                    //slope_E = atan(((z2[i]+h2[i]+beta*u2[i]*u2[i]/2/g)-(z2[i+1]+h2[i+1]+beta*u2[i+1]*u2[i+1]/2/g))/dx);
        ...
        for (i = 1; i < X_GRID; i++) {
            ...
            if (h1[i] < stop) { 
                ...
                double slope_E = slope;
                if (slope_E < 0) {
                    //slope_E = atan(((z1[i]+h1[i]+beta*u1[i]*u1[i]/2/g)-(z1[i+1]+h1[i+1]+beta*u1[i+1]*u1[i+1]/2/g))/dx);
